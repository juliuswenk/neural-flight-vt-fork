<script lang="ts">
  ...
  import { RadioManager } from "$lib/experiences/berlin-flight/lennard/radio/radio-manager";
  import { RADIO_STATIONS } from "$lib/experiences/berlin-flight/lennard/radio/radio-config";
  import { RADIO } from "$lib/experiences/berlin-flight/lennard/radio/radio-config";
  ...
  onMount(() => {
    ...
    updateVisuals = () => {
      ...
      for (const st of stationStates) {
        ...
        innerRing.material.opacity = 0.15;
  ...
  function applyStation(index: number) {
    ...
    const station = radioManager.getStation(index);
    if (!station) return;
