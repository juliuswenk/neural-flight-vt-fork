...
import { WERKSCHAU_CONE_DEBUG_MARKERS } from "../constants";
...
const scratchTipScale = new THREE.Vector3(
  WERKSCHAU_CONE_DEBUG_MARKERS.TIP_SIZE,
  WERKSCHAU_CONE_DEBUG_MARKERS.TIP_SIZE,
  WERKSCHAU_CONE_DEBUG_MARKERS.TIP_SIZE,
);
...
export class WerkschauConeDebugMarkers {
  ...
  constructor() {
    this.tipMesh = new THREE.InstancedMesh(
      ...
      new THREE.MeshBasicMaterial({
        color: WERKSCHAU_CONE_DEBUG_MARKERS.TIP_COLOR,
        transparent: true,
        opacity: WERKSCHAU_CONE_DEBUG_MARKERS.OPACITY,
        depthWrite: false,
      }),
      WERKSCHAU_CONE_DEBUG_MARKERS.MAX_MARKERS,
    );
    this.axisMesh = new THREE.InstancedMesh(
      new THREE.CylinderGeometry(0.5, 0.5, 1, 6, 1, true),
      new THREE.MeshBasicMaterial({
        color: WERKSCHAU_CONE_DEBUG_MARKERS.AXIS_COLOR,
        transparent: true,
        opacity: WERKSCHAU_CONE_DEBUG_MARKERS.OPACITY,
        depthWrite: false,
      }),
    ...
    this.group.name = "WerkschauConeDebugMarkers";
  ...
  public update(cones: readonly WerkschauConeVolume[]): number {
