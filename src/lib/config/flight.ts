...
export const M5_BRIDGE = {
  ...
  INVERT_PITCH: false,
  INVERT_ROLL: false,
  MOUNTING_PITCH_USES_ROLL: true,
  MOUNTING_ROLL_USES_PITCH: true,
  ...
  for (const [key, value] of Object.entries(settings)) {
    switch (key) {
      ...
      case "calibrationNeutralPitch":
        if (typeof value === "number") {
          m5BridgeRuntimeConfig.calibrationNeutralPitch = clamp(
            value,
            -180,
            180,
          );
      ...
      case "calibrationNeutralRoll":
        if (typeof value === "number") {
          m5BridgeRuntimeConfig.calibrationNeutralRoll = clamp(
            value,
            -180,
            180,
          );
